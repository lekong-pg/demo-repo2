...
`ifdef EMIF_FIRMWARE_FULLCAL
    ...
    generate                                                                                                                                                           \
       for (_pin = 0; ch_idx < MEM_NUM_CHANNELS_PER_IO96 && _pin < $size(``SIG``_LOCATION_BYTE) && _pin < ``SIG``_WIDTH; ++_pin) begin : g_``SIG``_bypass_``ch_idx``  \
...
`else
    ...
    `define out_se(ch_idx, SIG, sig, source)                                                                                                                                 \
       generate                                                                                                                                                              \
          for (_pin = 0; ch_idx < MEM_NUM_CHANNELS_PER_IO96 && (_pin+ch_idx) < $size(``SIG``_LOCATION_BYTE) && _pin < ``SIG``_WIDTH; ++_pin) begin : g_``SIG``_``ch_idx``_out        \
             tennm_ph2_io_obuf # (                                                                                                                                           \
...
module emif_hps_emif_ph2_phy_arch_fp_600_jc3j7xi_phy_arch_fp_atom_inst_bufs_mem #(
   ...
   import emif_hps_emif_ph2_phy_arch_fp_600_jc3j7xi_atom_attr_pin_locations::*;
   import emif_hps_emif_ph2_phy_arch_fp_600_jc3j7xi_atom_attr_bufs_mem::*;
