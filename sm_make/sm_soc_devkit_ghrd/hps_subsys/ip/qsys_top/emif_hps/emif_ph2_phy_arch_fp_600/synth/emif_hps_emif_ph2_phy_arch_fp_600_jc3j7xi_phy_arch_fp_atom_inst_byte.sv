...
module emif_hps_emif_ph2_phy_arch_fp_600_jc3j7xi_phy_arch_fp_atom_inst_byte #(
   ...
   timeunit 1ns;
   timeprecision 1ps;
   import emif_hps_emif_ph2_phy_arch_fp_600_jc3j7xi_atom_attr_byte::*;
   ...
   assign { i_phy_tx_wrdata_en0,
            ...
            i_phy_rx_senseampen,
            i_phy_sdll0_rx_d0pienable, i_phy_sdll0_rx_d0rcvenpre, i_phy_sdll0_rx_d0reset, i_phy_sdll0_rx_d0sdlparkvalue,
            i_phy_sdll1_rx_d0pienable, i_phy_sdll1_rx_d0rcvenpre, i_phy_sdll1_rx_d0reset, i_phy_sdll1_rx_d0sdlparkvalue,
            i_dqsmode,
   ...
   always_comb begin
      ...
      if (PIN_RX_USAGE[ID][5] == "PIN5_RX_USAGE_DQS")
      begin
      ...
      if (PIN_RX_USAGE[ID][7] == "PIN7_RX_USAGE_DQS")
      begin
         // synthesis translate_off
...
`define byte_pinXY_diff_params(X,Y)                                                             \
            .pin``X````Y``_diff_bus_hold              (PIN_DIFF_BUS_HOLD       [ID][``X``/2]),  \
            .pin``X````Y``_diff_io_standard           (PIN_DIFF_IO_STANDARD    [ID][``X``/2]),  \
            .pin``X````Y``_diff_rx_equalization       (PIN_DIFF_RX_EQUALIZATION[ID][``X``/2]),  \
            .pin``X````Y``_diff_rzq_id                (PIN_DIFF_RZQ_ID         [ID][``X``/2]),  \
            .pin``X````Y``_diff_schmitt_trigger       (PIN_DIFF_SCHMITT_TRIGGER[ID][``X``/2]),  \
            .pin``X````Y``_diff_termination           (PIN_DIFF_TERMINATION    [ID][``X``/2]),  \
            .pin``X````Y``_diff_usage_mode            (PIN_DIFF_USAGE_MODE     [ID][``X``/2]),  \
            .pin``X````Y``_diff_vref                  (PIN_DIFF_VREF           [ID][``X``/2]),  \
            .pin``X````Y``_diff_weak_pull_down        (PIN_DIFF_WEAK_PULL_DOWN [ID][``X``/2]),  \
            .pin``X````Y``_diff_weak_pull_up          (PIN_DIFF_WEAK_PULL_UP   [ID][``X``/2]),  
`define byte_pinX_params(X)                                                                \
            .pin``X``_direction                   (PIN_DIRECTION             [ID][``X``]), \
            .pin``X``_pcomp                       (PIN_PCOMP                 [ID][``X``]), \
            .pin``X``_rx_bus_hold                 (PIN_RX_BUS_HOLD           [ID][``X``]), \
            .pin``X``_rx_equalization             (PIN_RX_EQUALIZATION       [ID][``X``]), \
            .pin``X``_rx_gpio                     (PIN_RX_GPIO               [ID][``X``]), \
            .pin``X``_rx_io_standard              (PIN_RX_IO_STANDARD        [ID][``X``]), \
            .pin``X``_rx_rzq_id                   (PIN_RX_RZQ_ID             [ID][``X``]), \
            .pin``X``_rx_sampler_mode             (PIN_RX_SAMPLER_MODE       [ID][``X``]), \
            .pin``X``_rx_schmitt_trigger          (PIN_RX_SCHMITT_TRIGGER    [ID][``X``]), \
            .pin``X``_rx_termination              (PIN_RX_TERMINATION        [ID][``X``]), \
            .pin``X``_rx_usage                    (PIN_RX_USAGE              [ID][``X``]), \
            .pin``X``_rx_usage_mode               (PIN_RX_USAGE_MODE         [ID][``X``]), \
            .pin``X``_rx_vref                     (PIN_RX_VREF               [ID][``X``]), \
            .pin``X``_rx_weak_pull_down           (PIN_RX_WEAK_PULL_DOWN     [ID][``X``]), \
            .pin``X``_rx_weak_pull_up             (PIN_RX_WEAK_PULL_UP       [ID][``X``]), \
            .pin``X``_tx_equalization             (PIN_TX_EQUALIZATION       [ID][``X``]), \
            .pin``X``_tx_io_standard              (PIN_TX_IO_STANDARD        [ID][``X``]), \
            .pin``X``_tx_open_drain               (PIN_TX_OPEN_DRAIN         [ID][``X``]), \
            .pin``X``_tx_rate                     (PIN_TX_RATE               [ID][``X``]), \
            .pin``X``_tx_rzq_id                   (PIN_TX_RZQ_ID             [ID][``X``]), \
            .pin``X``_tx_slew_rate                (PIN_TX_SLEW_RATE          [ID][``X``]), \
            .pin``X``_tx_termination              (PIN_TX_TERMINATION        [ID][``X``]), \
            .pin``X``_tx_usage                    (PIN_TX_USAGE              [ID][``X``]), \
            .pin``X``_tx_usage_mode               (PIN_TX_USAGE_MODE         [ID][``X``]), 
